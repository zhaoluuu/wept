...
const videoComponent = {
  ...
  bindEvent() {
    ...
    NativeViewBridge.on('operateNativeView', data => this.actionChanged(data))
    NativeViewBridge.on('updateNativeView', data => this.setData(data))

    dom.addListener('play', e => {
      e._hasListeners = true;
      NativeViewBridge.publish('onNativeViewEvent', {
    ...
    NativeViewBridge.on('onAppEnterBackground', () => {
      if (!dom.paused) {
        this.actionChanged({
          method: 'pause'
        })
    ...
    NativeViewBridge.on('onAppEnterForeground', () => {
      if (forcePaused) {
        this.actionChanged({
          method: 'play'
        })
