...
function activate(context) {
    console.log('[EngineTutorial]extension activate');
    const engineDisposable = vscode.commands.registerCommand('workbench.engineTutorial', ({ data = {} }) => {
        onEngineRemoteMessage(data);
    });
    context.subscriptions.push(engineDisposable);
    const devtoolsDisposable = vscode.commands.registerCommand('workbench.devtoolsTutorial', ({ data = {} }) => {
        onEngineRemoteMessage(data);
    });
    context.subscriptions.push(devtoolsDisposable);
}
exports.activate = activate;
function onEngineRemoteMessage(data = {}) {
    console.log('[EngineTutorial]extension onRemoteMessage: ', data);
    ...
    switch (action) {
        case 'highlight':
            ...
            tips.forEach((tip = {}) => {
                ...
                decorationType = instance.createDecorationType({
                    after: {
                        contentText: tip.tipText ? `// [引导提示]: ${tip.tipText}` : '',
...
function onDevtoolsRemoteMessage(data = {}) {
    console.log('[DevtoolsTutorial]extension onRemoteMessage: ', data);
}
